(function () {
  ...
  function attachListener(input) {
    ...
    input.addEventListener('change', async (e) => {
      // ── Blocca il loop: se stiamo già processando, ignora ──
      if (input.dataset.pdfOptimizerProcessing === 'true') return;
  ...
  async function processFiles(input, allFiles, toProcess) {
    ...
    // Segna come "in elaborazione" per evitare loop
    input.dataset.pdfOptimizerProcessing = 'true';
    ...
    try {
      ...
      // Sostituisce i file senza triggerare il listener
      ...
      // Dispatcha evento — il flag processing impedisce il loop
      ...
      // Rimuove il flag dopo un tick — la pagina ha ricevuto l'evento
      setTimeout(() => { input.dataset.pdfOptimizerProcessing = 'false'; }, 100);
    }
  }
    ...
    return new Promise((resolve) => {
      ...
      img.onload = () => {
        ...
        try {
          ...
          canvas.toBlob((blob) => {
            // Se non migliora restituisce l'originale — nessun blocco
